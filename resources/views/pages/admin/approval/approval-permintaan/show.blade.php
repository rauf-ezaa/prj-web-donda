@extends('layouts.app')
@php $currentPageTitle = 'Verifikasi Admin - permintaan'; @endphp
@section('content')
...
>
    ...
    @php
        $bannerConfig = match($permintaan->status_permintaan) {
            ...
            default => ['bg' => 'bg-gray-50 dark:bg-gray-800', 'text' => 'text-gray-600 dark:text-gray-400', 'icon' => 'ti-info-circle', 'label' => ucfirst($permintaan->status_permintaan)],
        };
    @endphp
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-5">
        {{ $permintaan->requestedBy->nama_karyawan ?? '-' }} · Jabatan  · {{ $permintaan->keperluan }}
				<br>
    </p>
            ...
            <div>
                ...
                <p class="text-xs {{ $bannerConfig['text'] }} mt-1 m-0">
                    @if ($permintaan->status_permintaan === 'approved')
                        Disetujui oleh {{ $permintaan->approvedBy->nama_karyawan ?? '-' }} pada {{ $permintaan->approved_at?->translatedFormat('d F Y, H:i') }}. Stok telah diperbarui.
                    @elseif ($permintaan->status_permintaan === 'rejected')
                        Ditolak oleh {{ $permintaan->approvedBy->nama_karyawan ?? '-' }} pada {{ $permintaan->approved_at?->translatedFormat('d F Y, H:i') }}.
                        @if ($permintaan->catatan_approval)
                            Alasan: {{ $permintaan->catatan_approval }}
                        @endif
                    @elseif ($permintaan->status_permintaan === 'menunggu_spv')
                        Diteruskan oleh admin {{ $permintaan->verifiedByAdmin->nama_karyawan ?? '-' }}, menunggu keputusan Anda.
                    @else
                        Status_permintaan saat ini: {{ str_replace('_', ' ', $permintaan->status_permintaan) }}
                    @endif
                </p>
            </div>
        </div>
...
<div x-data="verifikasiAdmin({ id: {{ $permintaan->id }} })" class="max-w-2xl mx-auto">
    ...
    <x-common.component-card title="Daftar Barang Yang Diminta">
...
function verifikasiAdmin({ id }) {
    return {
        ...
        async approve() {
            ...
            try {
                const res = await fetch(`/admin/permintaan/${this.id}/approve`, {
                    method: 'POST',
                    headers: {
        ...
        async reject() {
            ...
            this.isProcessing = true;
            try {
                const res = await fetch(`/admin/permintaan/${this.id}/reject`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
