...
<div class="p-4 md:p-6">
    ...
    <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Pilih status untuk menampilkan daftar peminjaman.
            </p>
        </div>

							@if($draftAktif)
							<div class="flex flex-col items-end gap-1">
									<a href="{{ route('peminjaman.draft', $draftAktif->id) }}">
											<x-ui.button size="md" variant="primary">
													Lanjutkan Draft ({{ $draftAktif->kode_peminjaman }})
											</x-ui.button>
									</a>
							...
							@else
							<form action="{{ route('peminjaman.draft.start') }}" method="POST">
									@csrf
    ...
    <div class="mb-5 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
        <form method="GET" action="{{ route('peminjaman.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-end">
                        ...
                        <tr class="text-gray-700 dark:text-gray-300">
                            ...
                            <td class="px-4 py-3 text-right">
                                @if($item->status === 'draft')
                                    <a href="{{ route('peminjaman.draft', $item->id) }}">
