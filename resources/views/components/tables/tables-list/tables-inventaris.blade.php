<div x-data="{
    ...
    dropdownOpen: null,
    get totalPages() {
        return Math.ceil(this.kib.length / this.itemsPerPage);
    },
    get paginatedkib() {
        const start = (this.currentPage - 1) * this.itemsPerPage;
        const end = start + this.itemsPerPage;
        return this.kib.slice(start, end);
    },
    ...
    <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">List Data KIB</h3>
            </div>
        ...
        <div class="overflow-hidden">
            <div class="max-w-full px-5 overflow-x-auto">
                <table id="TestTab" class="min-w-full">
                    <thead>
                        <tr class="border-gray-200 border-y dark:border-gray-700">
                            <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">No</th>
                            <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Jenis Inventaris</th>
                            <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Jumlah Jenis Barang</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <template x-for="(kib, index) in paginatedkib" :key="kib.id">
                            <tr>
                                <td class="py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white" x-text="((currentPage - 1) * itemsPerPage) + index + 1"
                                            ></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white" x-text="kib.kode_kib"></div>
                                        </div>
                                    </div>
                                </td>
                                 ...
                                 <td class="py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white" x-text="kib.klasifikasi"></div>
                                        </div>
                                    </div>
                                </td>

                               <td class="px-4 py-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white w-60 break-words"
                                        x-text="kib.deskripsi">
                                    </div>
                                </td>


                               <td class="px-4 py-4">
                                    <a :href="'/kartu-inventaris-barang/' + kib.id + '/edit'"
                                ...
                                <!-- <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 dark:text-gray-400" x-text="dataKib.price"></div>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 dark:text-gray-400" x-text="dataKib.category"></div>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="getStatusClass(dataKib.status)" x-text="dataKib.status"></span>
                                </td> -->
                                <!-- td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                                    <div class="flex justify-center relative">
                                        <x-common.table-dropdown>
                                            ...
                                            <x-slot name="content">
                                                <a :href="'/data-karyawan/' + dataKib.id + '/edit'" class="flex w-full px-3 py-2 font-medium text-left text-gray-500 rounded-lg text-theme-xs hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300" role="menuitem">
                                                    Edit 
                                                </a>
                                                <form x-bind:action="'/data-karyawan/' + dataKib.id" method="POST">
                                                    @CSRF
                                                    @method('DELETE')
