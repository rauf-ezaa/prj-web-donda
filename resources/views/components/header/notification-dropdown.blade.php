@php
    ...
    $jumlahNotif = $notifications->count();
@endphp

<div class="relative" x-data="{
    dropdownOpen: false,
    notifying: {{ $jumlahNotif > 0 ? 'true' : 'false' }},
    toggleDropdown() {
...
<div
    ...
    class="absolute right-0 mt-[17px] flex h-[300px] w-[400px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark sm:w-[400px]"
    style="display: none;"
>
        <!-- Dropdown Header -->
    ...
    @forelse($notifications as $notif)
        <li>
            <a href="{{ $notif['link'] }}"
               class="flex flex-col gap-1 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5">
                ...
                <span class="text-theme-xs text-gray-500 dark:text-gray-400">
                    Status: {{ $notif['status'] }} &middot; {{ $notif['waktu']->diffForHumans() }}
                </span>
            </a>
        </li>
    @empty
        <li class="flex flex-col items-center justify-center py-10 text-center">
            <span class="text-sm text-gray-400 dark:text-gray-500">Tidak ada notifikasi saat ini.</span>
        </li>
    @endforelse
</ul>
    </div>
