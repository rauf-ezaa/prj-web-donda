@extends('layouts.app')
@php $currentPageTitle = 'Verifikasi Saldo Awal'; @endphp
@section('content')
<div class="p-4 md:p-6 max-w-3xl mx-auto">
    <a href="{{ route('spv.saldo-awal.index') }}" class="mb-4 inline-block text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400">
        ← Kembali
    </a>
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-800 dark:text-white/90">{{ $saldoAwal->tanggal_pencatatan->translatedFormat('d F Y') }}</p>
    </div>
    ...
    <div class="flex gap-2">
        <form action="{{ route('spv.saldo-awal.verify', $saldoAwal->id) }}" method="POST"
        ...
        <form action="{{ route('spv.saldo-awal.reject', $saldoAwal->id) }}" method="POST"
              onsubmit="return confirm('Yakin tolak saldo awal ini?')">
            @csrf
